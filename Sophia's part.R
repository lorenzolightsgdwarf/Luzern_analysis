...
qplot(data=melt_score,x=variable,y=value,fill=Group,geom="boxplot")+
  ...
  geom_segment(aes(x=1,xend=2,y=2.25,yend=2.25))+annotate("text",x=1.5,y=2.3,label="z=-3.71, p<0.0001",size=5)+
  geom_segment(aes(x=1,xend=1,y=2.21,yend=2.25))+geom_segment(aes(x=2,xend=2,y=2.25,yend=2.21))+
  geom_segment(aes(x=1,xend=3,y=2.75,yend=2.75))+annotate("text",x=2,y=2.81,label="z=1.14, p=0.49",size=5)+
  geom_segment(aes(x=1,xend=1,y=2.75,yend=2.71))+geom_segment(aes(x=3,xend=3,y=2.75,yend=2.71))+
  geom_segment(aes(x=2,xend=3,y=2.5,yend=2.5))+annotate("text",x=2.5,y=2.56,label="z=4.85, p<0.0001",size=5)+
  geom_segment(aes(x=2,xend=2,y=2.46,yend=2.5))+geom_segment(aes(x=3,xend=3,y=2.46,yend=2.5))+theme(text = element_text(size=20))
...
m=glmer(data=experiment_score,value~Group+(1|variable),family=poisson())
summary(m)

qplot(data=experiment_score,x=Group,fill=as.factor(value),geom="bar")+ scale_x_discrete(labels=c("Tangible","Verbal"))  +scale_fill_brewer(palette = "Set1",name="Correct\nAnswers")+facet_wrap(~ Trial,nrow=2,
                                                                                 labeller=labeller(Trial = c("1"="Trial 1","2"="Trial 2","3"="Trial 3","4"="Trial 4","5"="Trial 5",
                                                                                                             "6"="Trial 6","7"="Trial 7","8"="Trial 8")))+labs(x="Feedback",y="# Participants")+theme(text = element_text(size=20))
...
experiment_score%>%dplyr::group_by(variable)%>%dplyr::summarise(median=median(value),iqr=IQR(value))

qplot(data=experiment_score,x=Group,fill=as.factor(value),geom="bar")+ scale_x_discrete(labels=c("Tangible","Verbal"))+scale_fill_brewer(palette = "Set1",name="Correct\nAnswers")+facet_wrap(~ Trial,nrow=2,
                                                                                                                                                                                 labeller=labeller(Trial = c("1"="Trial 1\n(Median 1, IQR: 1)","2"="Trial 2\n(Median 1, IQR: 1)","3"="Trial 3\n(Median 1, IQR: 1)","4"="Trial 4\n(Median 2, IQR: 2)","5"="Trial 5\n(Median 1, IQR: 1)",
                                                                                                                                                                                                             "6"="Trial 6\n(Median 2, IQR: 1)","7"="Trial 7\n(Median 1, IQR: 0)","8"="Trial 8\n(Median 3, IQR: 1)")))+labs(x="Feedback",y="# Participants")+theme(text = element_text(size=20))
...
JointFocus=merge(tmp,tmp2,by=c("Participant","Trial","Condition"))
rm(tmp)
rm(tmp2)
JointFocus$prop=JointFocus$Duration.x/JointFocus$Duration.y

m=lmer(data=JointFocus,prop~Condition+(1|Trial))
drop1(m,test="Chi")
summary(multcomp::glht(m,mcp(Condition="Tukey")))

qplot(data=JointFocus,x=as.factor(Trial),y=100*prop,fill=as.factor(Condition),geom="boxplot")+
  labs(x="Trial",y="% Fixation Time")+
  scale_fill_manual(values=c("#7cae00","#00bfc4", "#f8766d"), name="Condition", 
                    breaks = c("tangible", "static","Expert"),labels=c("Tangible","Verbal","Expert"))+
  theme(text = element_text(size=20))

JointFocus_plot=JointFocus%>%group_by(Trial,Condition)%>%summarise(meanP=mean(prop)*100,ci=100*sd(prop)*1.96/sqrt(n()),participants=n())
...
ggplot(JointFocus_plot, aes(x=as.numeric(Trial), y=meanP, colour=Condition)) + 
  ...
  scale_x_continuous(breaks = c(1:8))+
  scale_color_discrete(name="Condition", labels=c("Expert","Verbal","Tangible"))

###Learner Part

LearnerFocus=subset(JointFocus,Condition!="Expert")
...
###   Trial Score
 ...
 KL_Trial_Score=subset(KL,Condition!="Expert")
 tmp=experiment_score[,c("ID","Trial","value")]
 tmp$ID=as.factor(tmp$ID)
 tmp$Trial=as.factor(tmp$Trial)
 
 KL_Trial_Score=merge(KL_Trial_Score,tmp,by.x = c("Participant","Trial"),by.y=c("ID","Trial"))
 rm(tmp)
 
 qplot(data=subset(KL_Trial_Score,KL<1),y=value,x=KL)
 ...
 Saliency_Cumulative=Saliency%>%dplyr::group_by(Condition,Trial,primary.AOI)%>%dplyr::summarise(meanFixProp=sum(prop)/6)
 Saliency_Cumulative=Saliency_Cumulative%>%dplyr::group_by(Trial,Condition)%>%dplyr::mutate(meanFixProp=meanFixProp/sum(meanFixProp))
  
 AOI_Codes=read.csv("Models/AOI_codes.csv",header = F,na.strings = "qqqq")
 colnames(AOI_Codes)=c("AOI","r","b")
 Saliency_Cumulative=merge(Saliency_Cumulative,AOI_Codes,by.x = "primary.AOI",by.y = "AOI" );
 
 img=read.bmp("Models/6.AOI.res.BMP");
 width=dim(img)[1]
 height=dim(img)[2]
 
 Saliency_Subset=subset(Saliency_Cumulative,Condition=="tangible"&Trial==6)
 saliency_map=matrix(nrow = width,ncol = height,data = 0)
 
 for(r in c(1:nrow(test1))){
   red=Saliency_Subset[r,"r"]
   blue=Saliency_Subset[r,"b"]
   saliency_map[which(img[,,1]==red & img[,,3]==blue,arr.ind = T)]= Saliency_Subset[r,"meanFixProp"]
 }
 
 colnames(saliency_map)=c(1:height)
 rownames(saliency_map)=c(1:width)
 
 new_image=melt(as.matrix(isoblur(as.cimg(saliency_map),sigma = 5)))
 
 png("Heat-6-tangible",width=572,322);  
 qplot(data=new_image,x=Var2,y=Var1,fill=value^(0.5))+
   annotation_custom(rasterGrob(readPNG("Models/6.PNG"), 
                     interpolate=TRUE,width = unit(1,"npc"), height = unit(1,"npc")) ,
                     xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf)+
   scale_fill_viridis()+
   #scale_fill_gradientn(colours=c("white","yellow","red"))+
   scale_y_reverse(limits = c(width,0))+
   theme_void()+theme(legend.position="none")+
   geom_tile(alpha=0.5)+
   scale_x_continuous(limits = c(0, height))
...
Learn=merge(Learn,only_score[,c("ID","RLG")],by.x = "Participant",by.y="ID")
m=lmer(data=Learn,Perc~RLG+(1|Trial))
...
#KL divergence
  ...
  m=lmer(data=subset(KL,Condition!="Expert"&KL<1),KL~Condition+(1|Trial))
...
----------------------
  ...
  KL_Cumulative=Saliency%>%dplyr::group_by(Trial,Condition,primary.AOI)%>%dplyr::summarise(KL=mean(prop*log(prop/meanFixProp)))
  KL_Cumulative=subset(KL_Cumulative,Condition!="Expert")
  ...
  test=merge(subset(Saliency_Cumulative,Condition=="tangible"),AOI_Codes,by.x = "primary.AOI",by.y = "AOI" )
  ...
  qplot(data=new_image,x=Var2,y=Var1,fill=value)+ annotation_custom(rasterGrob(readPNG("Models/5.PNG"), interpolate=TRUE,width = unit(1,"npc"), 
                                                                        height = unit(1,"npc")) , xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf)+
    scale_fill_gradientn(colours=c("white","yellow","red"))+scale_y_reverse(limits = c(width,0))+theme_void()+geom_tile(alpha=0.5)+scale_x_continuous(limits = c(0, height))
